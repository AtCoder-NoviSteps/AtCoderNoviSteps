...
import { isValidUrl, isValidUrlSlug, sanitizeUrl, isSameOriginRedirect } from '$lib/utils/url';
...
describe('URL', () => {
  ...
  describe('sanitize URL', () => {
    ...
    // TODO: Add more test cases for URL sanitization, especially those that include characters that need to be escaped in HTML contexts (e.g., <, >, &, ", ').
  ...
  describe('isSameOriginRedirect', () => {
    ...
    describe('failure cases (should return false)', () => {
      ...
      test('rejects data: URL (opaque origin)', () => {
        expect(isSameOriginRedirect('data:text/html,<script>alert()</script>', requestOrigin)).toBe(
          false,
        );
      });
