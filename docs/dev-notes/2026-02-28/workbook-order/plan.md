...
- `use:action` はその要素が DOM にマウントされた瞬間に自動実行され、アンマウント時に戻り値の `destroy()` が自動呼び出しされる。`onMount` でも代替できるが `use:` は特定要素に直接紐づけるため可読性が高い。`{ destroy: cleanup }` の形で返すことで Svelte アクション型に合わせる
