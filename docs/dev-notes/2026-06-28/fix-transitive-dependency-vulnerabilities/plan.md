...
3. **vercel 54.14.5 → 54.18.1 では大半の内部パッケージバージョンが据え置き**: `@vercel/fun` は 1.3.0 のまま。path-to-regexp, tar, minimatch は修正されない

## 対処方針

### 方法: `overrides` で transitive dependency を強制的に安全なバージョンに引き上げる
...
### QA: `package.json` の `pnpm.overrides` vs `pnpm-workspace.yaml` の `overrides`
...
5. moderate / low の脆弱性も対処する場合は下記セクション参照
...
### moderate / low 脆弱性の調査結果

moderate 16件、low 5件。high と同様にほぼ全て devDependency 経由で本番影響なし。
...
#### 各 override 対象のリリースノート確認結果（moderate/low）
...
#### 追加 overrides（moderate/low 対応版）

```yaml
# pnpm-workspace.yaml に追加（high の overrides に加えて）
# --- moderate ---
overrides:
  # ... (既存の high overrides) ...
...
**注意**: `ts-deepmerge`（メジャー）、`uuid`（メジャー×2）、`srvx`（0.x マイナー）は override すると親ライブラリが壊れるリスクが高いため、上流の修正を待つのが安全。`joi` はパッチ範囲だが superforms の validation 挙動に影響する可能性があるため、適用する場合はテストで確認が必要。

### vercel のバージョンアップについて

- `54.14.5 → 54.18.0` へのアップデートも併せて行う（直接的な脆弱性修正は少ないが、全体的な改善がある）
- ただし vercel のアップデートだけでは high 脆弱性はほぼ解消されない。overrides が必須
...
pnpm audit            # high が 0 になることを確認
```

## QA: `pnpm audit` vs `pnpm outdated`
