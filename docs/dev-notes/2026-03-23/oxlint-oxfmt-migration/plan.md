...
| Svelte 5 runes グローバル            | ⚠️ 要設定     | `$state` 等を `.oxlintrc.json` の globals に手動追加が必要                |
| ESLint フラット設定形式              | ❌ 非互換     | oxlint 独自の `.oxlintrc.json` 形式を使用                                 |
...
- PR [oxc-project/oxc#19807](https://github.com/oxc-project/oxc/pull/19807): "Support .svelte file"（ドラフト、2026-02-27 オープン）
...
| ファイル            | 変更種別 | 内容                                                     |
| ------------------- | -------- | -------------------------------------------------------- |
| `.oxlintrc.json`    | 新規作成 | TS ルール設定 + Svelte 5 runes グローバル定義            |
...
5. **`any` の代替パターン（oxlint の `no-explicit-any` 対応）**
   - `(obj as any).prop = val` → intersection 型 `obj as T & { prop: U }` に 1 回キャストしてから代入
   - 関数の戻り値型が複雑な場合 → `ReturnType<typeof fn>` を使うと型と実装の乖離を防げる
   - テストの部分モックオブジェクトを完全型に渡す場合 → `as unknown as T`（二重キャスト）が意図を明示する慣用句
   - 型推論が届く場所の `: any` アノテーションは削除するだけで解決する（forEach パラメータ等）
