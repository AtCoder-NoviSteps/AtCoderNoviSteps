...
- [x] `workbooks.ts`: メソッドを広い→狭い順に並び替え
- [x] `workbooks.test.ts`: 実装の並び順に合わせて describe 順を変更 + `services/workbooks.test.ts` に vi.mocked ヘルパー関数を追加（`mockFindUnique` / `mockFindMany` / `mockCount`）

### WorkBookList コンポーネント整理（低リスク）

- [x] `WorkBookList.svelte`: ローカルの `type WorkbookTableProps` を削除し、`types/workbook.ts` から import
- [x] `WorkBookList.svelte`: `tableComponents` から Deprecated エントリ（TEXTBOOK / GENRE / THEME / OTHERS）を削除
  - `Partial<Record<WorkBookType, ...>>` 型注釈に変更
  - 参照箇所に `{#if TableComponent}` ガードを追加

### +page.server.ts 改善（低リスク）

- [x] `load` 内の3クエリを `Promise.all` で並列化
...
- [x] `CurriculumWorkBookList.svelte` を新規作成
  ...
  - `pnpm check` で確認済み（既存の WARNING のみ、新規 ERROR なし）
