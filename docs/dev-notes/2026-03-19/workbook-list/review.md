...
- [x] **#3** 3ファイルの `{#each}` に `(workbook.id)` キーを追加
  - 根拠: `{#if canRead(...)}` との組み合わせでフィルタ時に DOM 不整合が起きる

### 優先度: 中

- [x] **#2** `$effect` 内の `get(taskGradesByWorkBookTypeStore)` を `$taskGradesByWorkBookTypeStore` に変更
  - 根拠: Svelte 5 ドキュメント確認済み — `get()` は `$effect` のシグナル追跡をバイパスするため非リアクティブ

- [x] **#5** `create/+page.server.ts:71` の `console.log` を削除
  - 根拠: サービス層 (`workbooks.ts:158`) に同等のログがあり重複。title の直接ログは不要

- [x] **#6** `workbooks.test.ts` に `getWorkbookWithAuthor` のテストを追加
  - null return (workbook 未存在)
  - author 存在確認 (`isExistingAuthor` true/false)

- [x] **#7** タイブレーク用テスト (line 269-299) を実際のタイ状態に修正
  - 根拠: コメントと実装が不一致。Q6 タスクを 1→5 件に増やして Q7:5, Q6:5 の真のタイを実現

- [x] **#8** `workbooks_list.test.ts:43` のアサーションを強化
  - 9Q ボタンの active 状態 (`text-primary-700` クラス) を確認する

### 優先度: 低

- [x] **#10** `ext-left` → `text-left` (3ファイル)
- [x] **#11** object shorthand (4箇所) in `+page.server.ts`
- [x] **#12** `getTaskResult` の `?? []` を定数化
- [x] **#13** `.nth(1)` → `getByRole('button', { name: 'ログイン', exact: true })` に変更
  - 根拠: デフォルトの部分一致で「お試し用のアカウントでログイン」もヒットしていたため
- [x] **#14** `.claude/skills/session-close/instructions.md:52` typo 修正
