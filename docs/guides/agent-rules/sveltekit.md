...
**Never set `s-maxage` / `stale-while-revalidate` on a route whose content varies by session.** Shared caches key on URL + method + the headers named by `Vary` (RFC 9111), so without `Vary: Cookie` an anonymous response is served to logged-in users (#3862).
