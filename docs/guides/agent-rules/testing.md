...
- Vitest defaults `clearMocks` to `true`, so **never add `vi.clearAllMocks()`**. Clearing resets call history only; `mockResolvedValue` / `vi.when()` implementations persist, so each test must set the behavior it needs.
- `restoreMocks` still defaults to `false`; use `vi.restoreAllMocks()` for `vi.spyOn` when needed.
